...
export const entregasService = {
    createEntrega: async (entrega: Partial<Entrega>): Promise<Entrega> => {
        const response = await axiosInstance.post<Entrega>(`${API_URL}/entregas`, entrega);
        return response.data;
    },

    getEntregaByBoleta: async (idBoleta: number): Promise<Entrega> => {
        const response = await axiosInstance.get<Entrega>(`${API_URL}/entregas/boleta/${idBoleta}`);
        return response.data;
    }
};
