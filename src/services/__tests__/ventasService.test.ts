...
describe('ventasService', () => {
    ...
    describe('createBoleta', () => {
        it('should create a boleta and return it', async () => {
            const mockBoleta = { idBoleta: 1, montoTotal: 1000, estado: 'PENDIENTE' };
            (axiosInstance.post as any).mockResolvedValue({ data: mockBoleta });

            const result = await ventasService.createBoleta({ montoTotal: 1000 });

            expect(axiosInstance.post).toHaveBeenCalledWith(expect.stringContaining('/api/v1/ventas/boletas'), { montoTotal: 1000 });
    ...
    describe('getBoletasByCliente', () => {
        it('should fetch boletas for a specific client', async () => {
            const mockBoletas = [{ idBoleta: 1, montoTotal: 1000 }];
